...
if [ "$EUID" -eq 0 ]; then
        ...
        if [ -d "$app_directory" ]; then
            if [ "$(find "$backup_directory" -maxdepth 1 -type d | wc -l)" -gt 1 ]; then
                ...
                for app in "$app_directory"/*; do
                    echo "[BACKUP] - Enregistrement de ($(basename "$app")) dans le répertoire $backup_directory." & echo "[BACKUP - $date] - Enregistrement de ($(basename "$app")) dans le répertoire $backup_directory." >> $log_file;
                    file_export=$backup_directory/$(basename "$app")-backup-app
                    cp -r $app $file_export;
                    echo "[BACKUP] - Compression du répertoire $(basename "$app")-backup..." & echo "[BACKUP - $date] - Compression du répertoire $(basename "$app")-backup..." >> $log_file;
    ...
    echo "[1] Lancer les backups des bases de données.";
    echo "[2] Lancer les backups des applications.";
    echo "[3] Lancer les deux.";

    read choice;

    case $choice in
        1) rm -f ./*-backup-db* & sleep 0.5 & database_backup;;
        2) rm -f ./*-backup-app* & sleep 0.5 & app_backup;;
        3) rm -f ./*-backup-db* & rm -f ./*-backup-app* & sleep 0.5 & database_backup & app_backup;;
        *) echo "[BACKUP] Choix invalide.";;
    esac
else
    echo "[BACKUP] - Vous devez exécuter ce script en tant que `root`";
fi